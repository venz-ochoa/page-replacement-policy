#algorithm for most recently used page replacement
    ...
    for i in range(n):
        ...
        if page in frames:
            ...
            if '' in frames:
                ...
                #find MRU
                #this the only thing that changed from LRU LOL, just instead of min we used max LOL
                #everything else is the same
                #then we find the index with the largest last used index, which would be our MRU page
                #hence the use of max
                mru_idx = max(range(capacity), key=lambda x: last_used.get(frames[x], -1))
                frames[mru_idx] = page
...
if __name__ == "__main__":
    ...
    if length > 0 and capacity > 0:
        result = mru(pages, length, capacity)
